 ...
 * delta from the last scroll event this recognizer instance ever saw
 * (matching the pre-unification `ScrollGesture` controller's `prevScroll`,
 * which persisted for the recognizer's whole lifetime). Fires `onChange`
 * (with `onStart` on the first event of a burst) per scroll event,
 * `onEnd`/`onFinalize` 150ms after the last one (debounced, since there's no
 * native "scroll end" event).
 */
