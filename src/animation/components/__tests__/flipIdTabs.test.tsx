...
// These tests mirror a tab-indicator built out of `flipId` (buttons in a
...
// (already covered in flip.test.tsx). That structural difference matters:
...
describe('flipId — tab indicator (sibling swap within one tree)', () => {
    ...
    return (
      <div>
        <button data-testid="btn-a" onClick={() => setActive('a')}>
          {active === 'a' && (
            <animate.div data-testid="indicator-a" data-tab="a" flipId="ind" />
          )}
          A
        </button>
        <button data-testid="btn-b" onClick={() => setActive('b')}>
          {active === 'b' && (
            <animate.div data-testid="indicator-b" data-tab="b" flipId="ind" />
          )}
          B
        </button>
      </div>
  ...
  // Regression test: under React.StrictMode, a freshly-mounted flipId
...
describe('flipId — four tabs of varying widths, sequential switches', () => {
  const rects: Record<string, { left: number; top: number; width: number; height: number }> = {
