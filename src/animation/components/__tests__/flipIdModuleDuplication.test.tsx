...
// etc.), a plain module-level `flipIdRegistry` variable would end up as
...
// copy, silently turning every `flipId` transition into a no-op (no prior
...
// `flipId` element writes to is reachable via `globalThis`, and mutating
// it "from the outside" (standing in for a second module instance) is
// visible to code using the normal `animate` import.
describe('flipId registry lives on globalThis, not module scope', () => {
  const REGISTRY_KEY = '__REACT_UI_ANIMATE_FLIP_ID_REGISTRY__';
  ...
  it('registers claims on a Map reachable from globalThis', () => {
    ...
    render(<animate.div data-testid="el" flipId="global-check" />);
  ...
  it('an entry written directly on the globalThis map (standing in for a second module instance) still drives a real transition', async () => {
    ...
    render(<animate.div data-testid="el" flipId="cross-instance-id" />);
