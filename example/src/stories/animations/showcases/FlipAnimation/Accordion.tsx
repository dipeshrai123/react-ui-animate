...
const FAQS: FAQItem[] = [
  {
    id: 1,
    question: 'What is the `flip` prop?',
    answer:
  ...
  },
  {
    ...
    answer:
      'Yes — pass `flipOptions={withSpring({ stiffness, damping })}` or `flipOptions={withTiming({ duration })}`, the same descriptor helpers used by `animate` / `hover` / `unmount`.',
  },
  {
    id: 5,
    question: 'Any limitations?',
    answer:
      "Avoid also animating translateX/Y or scaleX/Y via `animate`/`hover`/`press`/`view` on the same element — those transforms are reserved internally for the flip transition itself.",
  },
];
...
const Example = () => {
  ...
  return (
    <ExampleLayout
      title="Flip Animation — Accordion"
      description="Expanding or collapsing one question instantly resizes it — and every other item in the list smoothly animates to its new position using the `flip` prop, with no manual height math."
