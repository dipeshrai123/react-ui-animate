...
Most animation libraries make you choose: a small core that leaves gestures to a second dependency, or a full-featured library that ships a lot of code you don't use. react-ui-animate gives you spring animation, FLIP-based flip transitions, unmount animations, and a full gesture set (hover, press, pan, drag, swipe, wheel, scroll, pinch, rotate) from one package, and it's still smaller than stitching the equivalent together from other libraries.
...
- **`flip` / `flipId`** — automatic FLIP-based flip and shared-element transitions, grouped with `FlipGroup`
...
- **`Unmount`** — animate a component out before it leaves the DOM
